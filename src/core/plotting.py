...
def plotting_context(context="notebook", font_scale=1.5, rc=None):
    ...
    Example:
    -------
     with plotting.plotting_context(font_scale=2):
        tears.create_full_tear_sheet(..., set_context=False)
...
def plot_monthly_returns_heatmap(returns, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    monthly_ret_table.rename(columns={i: m for i, m in enumerate(calendar.month_abbr)}, inplace=True)
...
def plot_drawdown_periods(returns, top=10, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    # print(df_drawdowns)
    for i, (peak, recovery) in df_drawdowns[["Peak date", "Recovery date"]].dropna(how="all").iterrows():
...
def plot_rolling_returns(
    ...
    **kwargs, optional
    ...
    is_cum_returns.plot(lw=2, color="forestgreen", alpha=0.6, label="Backtest", ax=ax, **kwargs)

    if len(oos_cum_returns) > 0:
        oos_cum_returns.plot(lw=2, color="red", alpha=0.6, label="Live", ax=ax, **kwargs)
...
def plot_rolling_beta(returns, factor_returns, legend_loc="best", ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    ax.set_ylabel("Beta")
    rb_1 = ep.rolling_beta(returns, factor_returns, rolling_window=APPROX_BDAYS_PER_MONTH * 6)
    rb_1.plot(color="steelblue", lw=2, alpha=0.6, ax=ax, **kwargs)
    rb_2 = ep.rolling_beta(returns, factor_returns, rolling_window=APPROX_BDAYS_PER_MONTH * 12)
