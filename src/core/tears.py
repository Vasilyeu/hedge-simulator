...
def create_simple_tear_sheet(
    ...
    if live_start_date is not None:
        live_start_date = ep_utils.get_utc_timestamp(live_start_date)
...
def create_returns_tear_sheet(
    ...
    - Plots: rolling returns (with cone), rolling beta, rolling sharpe,
        rolling Fame-French risk factors, drawdowns, underwater plot, monthly
...
def create_perf_attrib_tear_sheet(
    ...
    if factor_partitions is not None:
        ...
        perf_attrib.plot_factor_contribution_to_perf(perf_attrib_data, ax=plt.subplot(gs[current_section]))
        current_section += 1

        perf_attrib.plot_risk_exposures(portfolio_exposures, ax=plt.subplot(gs[current_section]))
