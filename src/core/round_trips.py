...
DURATION_STATS = [
    ...
    #  Instead of x.max() - x.min() this should be
...
def agg_all_long_short(round_trips, col, stats_dict):
    """Aggregate all long and short round_trips data"""
    stats_all = round_trips.assign(ones=1).groupby("ones")[col].agg(stats_dict).T.rename(columns={1.0: "All trades"})
    stats_long_short = (
        round_trips.groupby("long")[col].agg(stats_dict).T.rename(columns={False: "Short trades", True: "Long trades"})
    )

    return stats_all.join(stats_long_short)


def _groupby_consecutive(transactions, max_delta=pd.Timedelta("8h")):
    ...
    out = []
    for _, t in transactions.groupby("symbol"):
        t = t.sort_index()
        t.index.name = "dt"
        t = t.reset_index()
                ...
                roundtrips.append(
                    {
                        ...
                        "close_dt": dt,
                        "long": price < 0,  # pylint: disable=undefined-loop-variable
