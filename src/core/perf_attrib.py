...
def perf_attrib(
    ...
    return em_perf_attrib(returns, positions, factor_returns, factor_loadings)
...
def compute_exposures(positions, factor_loadings, stack_positions=True, pos_in_dollars=True):
    ...
    return em_compute_exposures(positions, factor_loadings)
...
def create_perf_attrib_stats(perf_attributes, risk_exposures):
    ...
    """
    summary = OrderedDict()
    total_returns = perf_attributes["total_returns"]
    specific_returns = perf_attributes["specific_returns"]
    common_returns = perf_attributes["common_returns"]
    ...
    annualized_returns_by_factor = [ep.annual_return(perf_attributes[c]) for c in risk_exposures.columns]
    cumulative_returns_by_factor = [ep.cum_returns_final(perf_attributes[c]) for c in risk_exposures.columns]
    ...
    if len(missing_stocks) > 0:
        if len(missing_stocks) > 5:
            missing_stocks_displayed = (
                f" {len(missing_stocks)} assets were missing factor loadings, "
                f"including: {', '.join(missing_stocks[:5].map(str))}..{missing_stocks[-1]}"
            )
        ...
        missing_stocks_warning_msg = (
            ...
            "\n"
            f"{positions[missing_stocks[:5].union(missing_stocks[[-1]])].mean()}.\n"
        )
    ...
    if len(missing_factor_loadings_index) > 0:
        if len(missing_factor_loadings_index) > 5:
            missing_dates_displayed = (
                f"(first missing is {missing_factor_loadings_index[0]}, "
                f"last missing is {missing_factor_loadings_index[-1]})"
            )
        else:
            missing_dates_displayed = list(missing_factor_loadings_index)

        warning_msg = (
            f"Could not find factor loadings for {len(missing_factor_loadings_index)} dates: {missing_dates_displayed} "
